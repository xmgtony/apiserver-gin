...
type AddAccountBillReq struct {
	BillDate       jtime.JsonTime `json:"bill_date"`       // 账单日期
	OriginIncident string         `json:"origin_incident"` // 账户产生的事由
	Amount         string         `json:"amount"`          // 账单金额（数据库单位分，接收前端值用string避免丢失精度）
	Relation       string         `json:"relation"`        // 与对方关系,如亲戚|同事|闺蜜
	ToName         string         `json:"to_name"`         // 对方姓名
	IsFollow       uint           `json:"is_follow"`       // 是否关注或者跟进，0不关注、1关注
